...
/**
Dummy class that will not be loaded from a JAR file, and can thus be tested as
such.
*/
...
*/
  ...
  def assertNoSuchAttribute (e: NoSuchElementException, name: String): Unit = {
    assert (e.getMessage === LibResource
...
/*
Verify that looking up the manifest for a null class reference fails with a
...
/*
Verify that looking up the manifest for a primitive class reference (which
do not have associated resources) fails with a NoSuchElementException.

Primitive types typically have no associated resources, because they are not
implemented in library code.
*/

      it ("must throw a NoSuchElementException if passed a class reference " +
      "having no associated resource") {
        val c = classOf [Double]
        val e = intercept [NoSuchElementException] {
          Manifest (c)
        }
        assert (e.getMessage === (LibResource
        ("util.resourceUrl.NoSuchElement", c.getName)))
      }

/*
Verify that looking up the manifest for a class that was not loaded from a JAR
file fails with a NoSuchElementException.

Test classes are loaded from .class files as-is, without being assembled into
JAR files. Consequently, we can use a reference to a locally-defined class
here.
*/

      it ("must throw a NoSuchElementException if passed a non-JAR file " +
      "class reference") {
        ...
        assert (e.getMessage === LibResource ("util.jarFile.NoSuchElement",
        url.toString))
      }

/*
Verify that looking up the manifest for a JAR file class reference that has no
manifest information fails with a NoSuchElementException.
*/

      it ("must throw a NoSuchElementException if passed a JAR file class " +
      "reference that has no manifest") {
        ...
        val jar = jarFile (url)
        assert (e.getMessage === LibResource
        ("util.Manifest.NoSuchElement.Missing", c.getName, jar.getName))
      }
...
/*
Verify that a NullPointerException is thrown if null is passed as the
attribute name.
*/
      ...
      it ("must throw a NullPointerException if passed a null attribute name")
      {
        new TestData {
          val e = intercept [NullPointerException] {
            javaManifest.getAttribute (null)
          }
...
Note: There is no need to test the message returned, as it is generated from
...
/*
Verify that a NoSuchElementException is thrown if asked to retrieve a
non-existent attribute.
*/

      it ("must throw a NoSuchElementException if passed a non-existent " +
      "attribute name") {
...
/*
Verify that a NoSuchElementException is thrown if the inception timestamp is
undefined.

Note: Most JAR files will not define this; this attribute is only defined by
Facsimile and its client projects. As a consequence the main Java runtime JAR
file will not have an inception timestamp attribute defined.
*/
...
/*
Verify that a DateTimeParseException is thrown if an invalid inception
timestamp has been defined.
*/
...
/*
Verify that a valid inception date is returned for correctly formatted
inception timestamp attributes.
*/

      it ("must return a date/time if timestamp is valid") {
        new TestData {
          val validDate =
          ZonedDateTime.parse ("2004-06-22T18:16-04:00[America/New_York]")
...
/*
Verify that a NoSuchElementException is thrown if the build timestamp is
undefined.
...
*/
      ...
      it ("must return a valid date/time if timestamp is valid") {
        new TestData {
          val validDate =
          ZonedDateTime.parse ("2014-08-14T13:40:00.000-04:00[America/New_York]")
...
*/
      ...
      it ("must return a valid title if defined") {
        new TestData {
          // TODO: Requires Scala 2.11
          // assert (javaManifest.specTitle === Properties.javaSpecName)
          assert (javaManifest.specTitle ===
...
/*
Verify that a NoSuchElementException is thrown if the specification vendor is
undefined.
*/
...
*/
      ...
      it ("must return a valid vendor if defined") {
        new TestData {
          ...
          assert (javaManifest.specVendor === System.getProperty
...
*/
      ...
      it ("must return a valid version if defined") {
        new TestData {
          // TODO: Requires Scala 2.11
          //assert (javaManifest.specVersion.toString ===
          //Properties.javaSpecVersion)
          assert (javaManifest.specVersion.toString === System.getProperty
