...
the ''equals contract'':
 ...
 -  ''Reflexive'': `x == x` should be true for various non-`null` `x` of the
    associated type.
 -  ''Symmetric'': `x == y` and `y == x` should both give the same, expected
...
/*
Function to compare the hash code for every value in a list to every other
value in a list.
*/
...
/*
Verify that the object never compares equal to null and does not result in a
NullPointerException being thrown.
*/
...
/*
Verify that each object compares unequal to every other different object and
vice versa.
*/

      it ("must return false when compared to different value (and false when "
      + "different value compared to this value)") {
        compare (equalsFixture.valueSample, false)
      }

/*
Verify that each object compares true to every other equal object and vice
versa.
*/
...
*/
      ...
      {
        assert (equalsFixture.valueSample.map (_.hashCode).toSet.size ===
        equalsFixture.valueSample.size)
      }
