...
/**
Value class representing a _cell_ element's flags.

@constructor Create a new _cell_ flags instance from an integer bit-field
value.

@param flags _Cell_ element's flags to be parsed.
*/
...
/**
Report whether the _attributes present_ flag is set.
...
@return `true` if attributes are present in the _cell_ definition; `false` if
not.
*/
  private[cell] def attributesPresent =(flags & 0x1) != 0

/**
Report whether the _joint data present_ flag is set.

If this flag is set, then the cell's definition includes joint (kinematic)
data; if clear, no joint data is present.

@return `true` if joint data is present in the _cell_ definition; `false` if
not.
*/
  private[cell] def jointDataPresent =(flags & 0x2) != 0

/**
Report whether the _geometry data present_ flag is set.

If this flag is set, then the cell's definition includes geometry data; if
clear, no geometry data is present.

@note In a _cell_ file definition, _geometry data_ refers to translation,
rotation and scaling data, rather than shape geometry.

@return `true` if geometry data is present in the _cell_ definition; `false`
if not.
*/
  private[cell] def geometryDataPresent =(flags & 0x4) != 0

/**
Report whether the _geometry data in matrix form_ flag is set.
...
This flag is ignored unless the _geometry data present_ flag is set.

@note In a _cell_ file definition, _geometry data_ refers to translation,
rotation and scaling data, rather than shape geometry.

@return `true` if geometry data is in matrix form in the _cell_ definition;
`false` if in non-matrix form.
*/
  private[cell] def geometryDataInMatrixForm =(flags & 0x8) != 0

/**
Report whether the _colors inherited_ flag is set.
...
@return `true` if colors are inherited from the _cell's_ parent; `false` if
not.
*/
  private[cell] def colorsInherited =(flags & 0x10) != 0

/**
Report whether the _bounding box present_ flag is set.
...
@note Bounding box information is read but discarded by _Facsimile_.

@return `true` if _bounding box data_ is present; `false` if not.
*/
...
/**
Read _cell_ flags from data stream.
