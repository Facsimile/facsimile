 ...
 *
 * Provide common information and functionality for applications built with the _Facsimile_ library.
 *
 * In order to compile, client applications *must* mix-in an [[AppInformation]]-implementation instance when
 * creating an application _object_ as well as either [[org.facsim.cli.CliApp]] or [[org.facsim.gui.GuiApp]] (or a
...
extends DelayedInit {
   ...
   * automatically by the _Scala_ compiler and passed to the `delayedInit` function, which then stores such code
   ...
   * in the normal manner, and this do not have _delayed initialization_.
   *
   * The [[DelayedInit]] trait has special behavior that is not entirely defined by its source code. Refer to the
   * _Scala API_ documentation for further details. This is complicated by the fact that, as of _Scala V2.11_, the
   * `DelayedInit` trait has been _deprecated_. There is a replacement
   ...
   *
   * @note This function is automatically invoked by code generated by the _Scala_ compiler; it must *never* be
   ...
   *
   * @note For _JavaFX_-based _GUI_ applications, this funciont will be executed on the _JavaFX Application
   * Thread_ after the primary stage has been created. Consequently, subclass constructor code can populate the primary
  ...
  /**
   * Retrieve the _Facsimile_ JAR file manifest.
   *
   * @return Manifest read from the _Facsimile_ JAR file.
   */
