...
@todo This is a work-in-progress and will change in the near future.  Full
version comparison will be implemented to permit version ordering, and
determinations of whether this is a snapshot version, a pre-release version,
etc.  For now, we just report back whatever string we're assigned.
...
@todo For now, the Version instance will be returned regardless of whether the
contents of '''version''' encodes a valid version number.  When full version
number processing is implemented, this will change.
