 ...
 * classes and functions with general applicability. Only simulation-related utilities are publicly accessible.
 *
 * @since 0.0
 */
package object util {
   ...
   * the new post-''Java 1.8'' `java.time` time classes ([[Instant]], [[ZonedDateTime]], etc) is cumbersome at best. The
   * former could be dispensed with completely if if wasn't for the fact that [[MessageFormat]] currently supports only
   * the [[Date]] class. This function makes working with the new time classes, and text message formatting, a little
