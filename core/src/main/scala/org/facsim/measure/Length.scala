...
extends Specific {
   ...
   *
   * This value is the internationally accepted rate for converting between meters and inches and is used internally to
   * convert between metric and Imperial/English length units. This value '''must not''' be modified!
   *
   ...
   *
   * @param measure ''Length'' measurement expressed in ''[[org.facsim.measure.Length.Meters]]''. This value must be
   * finite.
   *
   ...
   * @since 0.0
   */
  final class LengthUnits private[measure](converter: Converter, symbol: String)
