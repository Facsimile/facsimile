...
Records the frequency of observed value ranges by in ''bins''.  Values that are
lower than anticipated are stored in an ''underflow'' bin; those that are
higher than anticipated are stored in an ''overflow'' bin.

@constructor Create a new Histogram class instance.

@param minimumValue Minimum expected value.  Observed values lower than this
will be placed into an ''underflow'' bin.

@param bins Number of regular bins in the histogram.  Two additional bins, an
''underflow'' bin&mdash;storing values less than '''minimumValue'''&mdash;and
an ''overflow'' bin&mdash;storing values greater than '''minimumValue''' +
('''bins''' * '''binWidth'''&mdash;will be included in addition to these bins.
This argument must be a positive integer or an exception will be thrown.
...
@note Bin 0 is the ''underflow'' bin, which records the frequency of observed
values less than the specified minimum value.  Similarly, bin (bins + 1) is the
''overflow'' bin, which records the frequency of observed values that exceed
