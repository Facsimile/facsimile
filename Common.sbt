...
// range (e.g. 2004-2016); otherwise, since both years are the same, it will be a single year value (e.g. 2016). The
// return value is a string with the year range or value as appropriate.
...
// reverse of the web-site name (less the "www" prefix). Thus "http://facsim.org/" yields an organization ID/root
...
// warnings themselves will only be issued when -Yinline-warnings is specified, Scala will still emit a warning that
// inline warnings occurred, which is then treated as fatal by Xfatal-warnings. It seems that the only way around this,
// right now, is to disable -Xfatal-warnings. This is a known Scala compiler issue documented at
// https://issues.scala-lang.org/browse/SI-6723.
//
// As Xfatal-warnings is not in use, it's possible to have builds that generate tons of warnings, but which do not fail
// a build. This is unacceptable. Facsimile must build clean, without any errors or warnings, as a basic requirement for
// any release to be performned.
//
...
// required by Facsimile project dependencies) should not be explicitly included, as this can lead to versioning
...
//
// This information is required in order to deploy the project on the Sonatype OSS Nexus repository, which ensures that
// the project is synchronized with the Maven Central Repository.
//
// NOTES:
...
//    listed in a project's POM file. If such artifacts are required, then uncomment the "pomIncludeRepository" setting
//    so that the repositories from which they're referenced are ignored.
