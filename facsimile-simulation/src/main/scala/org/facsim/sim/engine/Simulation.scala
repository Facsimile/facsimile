...
final class Simulation[M <: ModelState[M]: Tag]:
   ...
   */
  given SimulationRef: Simulation[M] = this
  ...
  def at(delay: Time, priority: Priority = 0)(actions: Action[M]): SimulationAction[M] = State: s =>
    ...
    // describing why.
    if !s.runState.canSchedule then (s, Failure(EventScheduleStateException(s.runState)))
   ...
   *  @since 0.0
   */
  def takeUntilFailure(ts: Seq[SimulationAction[M]]): SimulationAction[M] =
    for
  ...
  private def initialState(initialModelState: M): SimulationState[M] =
    new SimulationState(initialModelState, 0L, None, BinomialHeap.empty[Event[M]], RunState.Initializing)
   ...
   *  week.
   *
   *  @param snapLength Duration, measured in simulation time, of each simulation _snap_ (simulation reporting period),
   *  with the first such snap starting after the warm-up period has completed. At the end of each snap, the statistics
   *  are reset and a report generated. If omitted, this defaults to one week.
   *
  ...
  private def updateCurrentEvent: SimulationAction[M] = State: s =>
    ...
    if !s.runState.canIterate then (s, Failure(EventIterationStateException(s.runState)))

    // Otherwise, if the event queue is empty, them we've run out of events.
    else if s.events.isEmpty then (s.update(newRunState = RunState.Terminated), Failure(OutOfEventsException))
  ...
  private def remainingEvents: SimulationAction[M] = State: s =>
    ...
    if result._2.isFailure || !result._1.runState.canIterate then result
  ...
  (initialization: Action[M]): (SimulationState[M], Try[Unit]) =
    ...
    val runToCompletion: SimulationAction[M] =
      for
        r <- takeUntilFailure:
          List[SimulationAction[M]](
