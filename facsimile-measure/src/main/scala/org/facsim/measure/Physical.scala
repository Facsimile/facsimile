...
/** Abstract base class for all ''Facsimile [[http://en.wikipedia.org/wiki/Physical_quantity physical quantity]]''
  * elements.
  *
  * Physical quantity types include specific types (such as ''[[org.facsim.measure.Angle]]'',
  * ''[[org.facsim.measure.Length]]'', ''[[org.facsim.measure.Time]]'', etc.), as well as a
...
abstract class Physical protected[measure] {
  ...
  extends Equals {
    ...
    /** Multiply this measurement by specified factor.
      ...
      *
      * @return Product of this measurement multiplied by `factor`. The result is of the same physical quantity family
      * as this measurement.
      *
      * @throws IllegalArgumentException if the result is not finite or is invalid for these units.
      *
  ...
  /** Abstract base class for all physical quantity measurement units.
    ...
    * of [[org.facsim.measure.Physical.PhysicalMeasure]] subclasses, with each subclass corresponding to each
    * measurement type.) For example, the ''SI'' standard unit of measure for ''time'' is the ''second''; consequently,
    * ''Facsimile'' stores and calculates all time quantities, internally, in ''seconds''. Adoption of the ''SI''
    * standard units simplifies the implementation of physics calculations within ''Facsimile'' and provides a
    * clearly-defined basis for developing simulation models of the real-world. (Many other simulation modeling tools
    * suffer from ''unit of measure confusion'', both internally and externally, creating a wide variety of problems.)
    *
