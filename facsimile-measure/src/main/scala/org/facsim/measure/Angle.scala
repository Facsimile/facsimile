 ...
 * ''[[http://en.wikipedia.org/wiki/SI SI]]'' standard unit of plane angle measure.
 *
...
extends Specific {
   ...
   *
   * @note The authors of ''Facsimile'' prefer the use of ''[[org.facsim.measure.Angle.τ]]'' to ''π'', because the
   * former is significantly easier to work with.
   *
  ...
  /**
   * A proposed mathematical constant, ''[[http://en.wikipedia.org/wiki/Turn_(geometry)#Tau_proposal τ]]'' (tau), in the
   * form of an angle value.
   *
   * ''τ'' is defined as the ratio of the circumference of a circle to its radius.
   *
   * This constant, whose value is 2''π'', is preferred throughout ''Facsimile'' for its simplicity and ease-of-use
   * compared to ''π''. A ''radian'' measure of ''τ'' is one ''turn'' (or ''revolution''), so fractions of ''τ'' are
   ...
   *
   * Plane angles are ''unitless'' measurements, with radian measure defined as the ratio one [[Length]] (the arc of the
   * circumference subtended by the angle) to another (the radius), resulting in a unitless value.
   */
  protected[measure] val family = Family.Unitless
   ...
   *
   * @param measure Measurement, in radians, to be converted into a new measure.
   *
   * @return `measure` in the form of an Angle measurement.
   */
   ...
   *
   * @throws IllegalArgumentException if `sine` is `NaN` or is outside of the range [-1, 1].
   *
   * @since 0.0
   */
   ...
   *
   * @throws IllegalArgumentException if `cosine` is `NaN` or is outside of the range [-1, 1].
   *
   * @since 0.0
   **/
   ...
   *
   * @note The [[org.facsim.measure.Angle.atan2(org.facsim.measure.Length.Measure, org.facsim.measure.Length$.Measure)]]
   * function should be preferred where applicable, since it returns a fuller range of angle values.
   *
   ...
   * ''θ'').
   *
   * @note This function should be preferred over the [[org.facsim.measure.Angle.atan(Double)]] function where
   * applicable.
   *
   ...
   *
   * @param measure ''Plane angle'' measurement expressed in ''[[org.facsim.measure.Angle.Radians]]''. This value must
   * be finite.
   *
   ...
   * @since 0.0
   */
  final class AngleUnits private[measure](converter: Converter, symbol: String)
