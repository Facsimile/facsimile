...
extends Physical {
  ...
  /** Generic ''Facsimile [[http://en.wikipedia.org/wiki/Physical_quantity physical quantity]]'' measurement class.
    ...
    * is divided by a [[org.facsim.measure.Time]], then the result is a ''generic velocity'', which can be converted to
    * a [[org.facsim.measure.Velocity]]. However a ''time'' multiplied by a ''time'', results in a generic
    * ''time-squared''  measurement has no equivalent specific class and can be expressed only in generic form.
    *
