 ...
 *
 * For example, when a [[org.facsim.measure.Length]] measurement is divided by a [[org.facsim.measure.Time]]
 * measurement, the result is a [[org.facsim.measure.Velocity]] measurement. However creating tables that define the
 * result of each pair of operands for each operation would be time-consuming, error prone and non-exhaustive.
 *
 * A simpler approach is to create a generic measurement, capturing the family's characteristics, which can be converted
 * to the corresponding specific physical measurement for that family by implicit conversion. If the generic
 * measurement's family is not identical to the specific measurement it is being converted to, an exception will result.
 *
...
extends Physical {
   ...
   *
   * @throws org.facsim.measure.GenericConversionException if `measure` is not ''unitless''.
   *
   * @since 0.0
   */
   ...
   *
   * @todo There are currently no checks to ensure that `measure` is within a valid range for the associated family.
   ...
   *
   * @throws IllegalArgumentException if `measure` is not finite.
   */
  private[measure] def apply(measure: Double, family: Family) = new GenericMeasure(measure, family)
   ...
   * physical quantity family&mdash;when a specific equivalent exists. For example, if a [[org.facsim.measure.Length]]
   * is divided by a [[org.facsim.measure.Time]], then the result is a ''generic velocity'', which can be converted to a
   * [[org.facsim.measure.Velocity]]. However a ''time'' multiplied by a ''time'', results in a generic ''time-squared''
   * measurement has no equivalent specific class and can be expressed only in generic form.
   *
   * Attempts to convert ''generic'' measurements to measurements of a different physical quantity family will resulting
   ...
   * @param family Physical quantity family to which `measure` belongs.
   *
   * @throws IllegalArgumentException if `measure` is not finite or is invalid for the associated `family`.
   *
   * @see [[http://en.wikipedia.org/wiki/SI International System of Units]] on [[http://en.wikipedia.org/ Wikipedia]].
   *
   * @since 0.0
   */
  final class GenericMeasure private[measure](measure: Double, override val family: Family)
  extends PhysicalMeasure(measure) {
     ...
     * @inheritdoc
     */
    protected[measure] override def createNew(newMeasure: Double) = apply(newMeasure, family)
  }
