...
/** Offset converter.
  ...
  * ''SI'' unit values) by adding the specified constant value; they are ''exported'' (converted from ''SI'' unit
  * values) by subtracting the same constant value.
  *
  * @constructor Create new offset converter from the specified `offset`.
  *
  * @param offset Constant value that, when added to specific measurements in the corresponding non-''SI'' units,
  ...
  */
private[measure] final class OffsetConverter(offset: Double)
extends Converter {
  ...
  /** @inheritdoc
    */
  private[measure] override def importValue(value: Double): Double = value + offset

  /** @inheritdoc
    */
  private[measure] override def exportValue(value: Double): Double = value - offset
}