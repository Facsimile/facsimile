 ...
 * ''SI'' unit values) by adding the specified constant value; they are ''exported'' (converted from ''SI'' unit values)
 * by subtracting the same constant value.
 *
...
extends Converter {
   ...
   * @inheritdoc
   */
  private[measure] override def importValue(value: Double): Double = value + constant

  /**
   * @inheritdoc
   */
  private[measure] override def exportValue(value: Double): Double = value - constant
}