...
/** Trait for implicit objects allowing values of the associated type to be added and subtracted.
 *
 *  @tparam A Type of value being added and/or subtracted.
 *
 *  @since 0.0
 */
trait Addition[A] {
   ...
   *
   *  @return Result of the addition of `b` to `a`, wrapped in a [[scala.util.Success]] if successful, or a
   *  [[scala.util.Failure]] otherwise.
   *
   *  @since 0.0
   */
  def add(a: A, b: A): Try[A]
