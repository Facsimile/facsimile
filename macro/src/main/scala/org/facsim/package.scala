...
/*
The Scala "macros" language feature is currently experimental, and needs to be enabled via this import statements.
*/
...
Normally, a `NullPointerException` will be thrown by the ''Java'' virtual machine (''JVM'') if an attempt is made to
dereference a `null` pointer. However, if a function takes an object reference argument and that argument is not
dereferenced until after the function has returned, then the function must verify that the reference is non-`null` as
one of its preconditions; this function makes such precondition verification simpler.

Furthermore, even if the ''JVM'' can be relied upon to throw this exception, performing this verification explicitly is
regarded as good practice. One reason is that exceptions thrown by the ''JVM'' provide limited explanation to the user
as to their cause; this function provides an explanation automatically.
...
@param isValid Flag representing the result of a condition determining the validity of `arg`. If `true`, function merely
returns; if `false` an `IllegalArgumentException` is raised.
...
argument belongs), which creates confusion when identifying failing arguments, and testing that failed argument messages
match expected messages. This function removes class argument prefixes so that the raw argument name is returned.
...
/*
Convert the argument into a string that represents the expression that was passed to the requireNonNull macro - we'll
output that as part of the exception.
*/
...
*/
    ...
    reify {
      if (arg.splice eq null) throw new AssertionError (LibResource (AssertNonNullKey, cleanArgName (argString.splice)),
      null)
    }
  }
...
@param arg Argument whose value is to be tested. If this argument evaluates to `null`, then a
[[scala.NullPointerException]] is thrown by the macro implementation, together with the name of the failed argument.
...
*/
    ...
    reify {
      if (arg.splice eq null) throw new NullPointerException (LibResource (RequireNonNullKey, cleanArgName
      (argString.splice)))
    }
  }
...
[[scala.IllegalArgumentException]] is thrown by the macro implementation, together with the name of the failed argument.
...
*/
    ...
    reify {
      if (!isValid.splice) throw new IllegalArgumentException (LibResource (RequireValidKey, cleanArgName
      (argString.splice), arg.splice.toString))
    }
  }
