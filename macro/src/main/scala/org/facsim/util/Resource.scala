...
Note that we cannot use the requireNonNull macro here, as this class is part of the macro compilation unit. Instead, we
have to do the equivalent manually (this is one reason why macros need to be compiled in-line, because things can easily
get silly).

Also, note that we have to explicitly pass the name of the argument, since this isn't a macro.
*/

  if (bundleName eq null) throw new NullPointerException (LibResource (RequireNonNullKey, "bundleName"))

/**
Resource bundle.

The locale of the bundle defaults to the JVM's default locale; if a locale-specific resource bundle matching the default
locale is available, then it will be loaded - otherwise, the "en_US" (United States English) resource bundle will be
loaded.
*/
...
Retrieves the string resource corresponding to the supplied key from the application resource bundle. Optional arguments
are used to populate the formatted version of the resulting string, which will be in the locale that best matches the
user's preference.

@param key Key used to identify the string resource to be retrieved.

@param arguments Arguments to be merged into, and formatted as part of, the resulting string resource. May be omitted if
no arguments are required. Note that any Scala type values (Double, Int, etc.) will be boxed into equivalent Java type
wrappers (java.lang.Double, java.lang.Int, etc.).
...
@throws java.lang.IllegalArgumentException if the retrieved string is invalid or if the `arguments` supplied are of the
...
*/
    ...
    if (key eq null) throw new NullPointerException (LibResource (RequireNonNullKey, "key"))
...
*/
    ...
    MessageFormat.format (bundle.getString (key), arguments.map (_.asInstanceOf [AnyRef]): _*) // scalastyle:ignore
  }
}
