 ...
 *  @since 0.0
 */
final class BinomialHeap[A] private(private val rootTree: BinomialTree[A])(using private val ordering: Ordering[A],
...
derives CanEqual:
  ...
  private def link(n1: BinomialTreeNode[A], n2: BinomialTreeNode[A]): BinomialTreeNode[A] =
    ...
    if ordering.lteq(n1.root, n2.root) then n1.copy(rank = n1.rank + 1, children = n2 :: n1.children)
  ...
  private def insert(n: BinomialTreeNode[A], t: BinomialTree[A]): BinomialTree[A] = t match
    ...
    case nh :: tt =>
      ...
      if n.rank < nh.rank then n :: nh :: tt
  ...
  private def meld(t1: BinomialTree[A], t2: BinomialTree[A]): BinomialTree[A] = (t1, t2) match
    ...
    case (n1 :: t1t, n2 :: t2t) =>
      ...
      if n1.rank < n2.rank then n1 :: meld(t1t, t2)
      ...
      else if n2.rank < n1.rank then n2 :: meld(t2t, t1)
  ...
  private def findMin(t: BinomialTree[A]): Option[A] = t match
    ...
    case n :: ts =>
      findMin(ts).map: x =>
        ...
        if ordering.lteq(n.root, x) then n.root
    ...
    case n :: tt =>
      ...
      def getMin(minN: BinomialTreeNode[A], rem: BinomialTree[A]): (BinomialTreeNode[A], BinomialTree[A]) = rem match
        ...
        case rn :: rt =>
          ...
          if ordering.lteq(minN.root, minRem.root) then (minN, rem)

          // Otherwise, remove the minimum node from the tail and prepend the previous minimum to the tail's remainder.
   ...
   *
   *  @param other Object being compared to this heap for equality.
   *
   *  @return `true` if `other` is a heap of the same type as this heap; `false` in all other cases (if `other` is not a
   *  heap, or if `other` is a heap of a different type).
   *
   *  @since 0.0
   */
  override def canEqual(other: Any): Boolean = other match

    // If other is a BinomialHeap, and the type of its elements is the same, then we can compare the two for equality.
    //
    // NOTE: This requires a direct comparison of the types of the two heaps, which we obtain through reflection; we
    // cannot match more explicitly, due to generic type erasure.
    case other: BinomialHeap[?] => tag =:= other.tag

    // If that is anything else (not a BinomialHeap, but a different object altogether), then we cannot compare for
    // equality.
    case _ => false
   ...
   *  @param other Object being compared to this heap for equality.
   *
   *  @return `true` if `other` is a heap of the same type as this heap and contains the same elements (the structures
   *  of the two heaps, however, may differ); `false` in all other cases (if `other` is not a heap, if `other` is a heap
   *  of a different type, or if the elements in the two heaps differ).
   *
   *  @since 0.0
   */
  @tailrec
  override def equals(other: Any): Boolean = other match
    ...
    // Is that a BinomialHeap that can be compared as equal to this heap? If not, return false. Otherwise, the heaps are
    // of the same type, so compare their contents.
    case that: BinomialHeap[?] =>
      if !that.canEqual(this) then false
      else

        // Remove the minimums from each heap and compare them.
        val (tm, tr) = cachedMinimumRemove
        val (om, or) = that.cachedMinimumRemove
        tm match
...
object BinomialHeap:
   ...
   *  @param ordering Ordering allowing elements of type `A` to be compared.
   *
   *  @param tag Tag providing information about `A` at runtime.
   *
   *  @return Empty heap.
   *
   *  @since 0.0
   */
  def empty[A](using ordering: Ordering[A], tag: Tag[A]): BinomialHeap[A] = new BinomialHeap[A](Nil)
   ...
   *  @since 0.0
   */
  def apply[A](as: A*)(using ordering: Ordering[A], tag: Tag[A]): BinomialHeap[A] =
